  ...
  gltf.scene.traverse((mesh) => {
    ...
    if (name.includes('bulb')) {
      ...
      if (isLineBulb) {
        ...
        // 多層球體，從內到外 opacity 遞減，製造漸淡效果
        const glowLayers = [
          { radius: baseRadius * 1.2, opacity: 0.45 },  // 0.20 → 0.45
          { radius: baseRadius * 1.8, opacity: 0.25 },  // 0.10 → 0.25
          { radius: baseRadius * 2.8, opacity: 0.12 },  // 0.05 → 0.12
          { radius: baseRadius * 4.0, opacity: 0.06 },  // 0.02 → 0.06
        ];
