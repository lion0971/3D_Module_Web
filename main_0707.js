...
const LINE_NOTIFY_URL = 'https://script.google.com/macros/s/AKfycbxzcEx2gm46YfqtDv20spcHxAYApHP8Le2IF_U9Cbg87tNoWzcmrxVi03FEi_VS1yiW/exec';

function sendLineNotification(deviceName) {
  const label = DEVICE_LABEL[deviceName] ?? deviceName;
  fetch(LINE_NOTIFY_URL, {
    method: 'POST',
    body: JSON.stringify({
      message: `⚠️ ${label} 已持續出水超過1分鐘，請確認是否忘記關閉！`
    })
  }).catch(err => console.warn('[LINE] 通知發送失敗', err));
}
...
function showWarning(deviceName) {
  ...
  sendLineNotification(deviceName);   // ← 新增這行
