...
warningOffBtn.onclick = () => {
  ...
  stopPollingRemoteClose(deviceName); // ★ 新增：手動關閉後就不用再輪詢了
 ...
 * @param {string} deviceName 完整裝置名稱，如 'faucet', 'faucet_2', 'shower_2'
 */
// ── Google Apps Script 網址（換成你自己的） ──
const LINE_NOTIFY_URL = 'https://script.google.com/macros/s/AKfycbyPX_VfcdVKjv3WiU8o6dEApaPjH0v625oDZvMXqkoP7OxNycsR-BON2tLy-QFsjc-8PQ/exec';

const currentWarningToken = {};

function sendLineNotification(deviceName, token) {
  const url = `${LINE_NOTIFY_URL}?action=notify&device=${encodeURIComponent(deviceName)}&session=${encodeURIComponent(token)}`;
  fetch(url).catch(err => console.warn('[LINE] 通知發送失敗', err));
}

const pollingIntervals = {};

function startPollingRemoteClose(deviceName, token) {
  stopPollingRemoteClose(deviceName);

  pollingIntervals[deviceName] = setInterval(async () => {
    try {
      const url = `${LINE_NOTIFY_URL}?action=poll&device=${encodeURIComponent(deviceName)}&session=${encodeURIComponent(token)}`;
      ...
      console.warn('[LINE] 輪詢查詢失敗', err);
    }
  }, 3000);
}

function stopPollingRemoteClose(deviceName) {
  if (pollingIntervals[deviceName]) {
    clearInterval(pollingIntervals[deviceName]);
    delete pollingIntervals[deviceName];
  }
}
...
function displayNextWarning() {
  ...
  warningText.innerHTML = `⚠️ 警告<br><span style="color:#ffdd00;font-size:22px">${label}</span>...`;
